package by.awesome.sup.config.security.authorization;

import by.awesome.sup.entity.common.project.Project;
import lombok.RequiredArgsConstructor;
import org.springframework.security.core.authority.SimpleGrantedAuthority;
import org.springframework.security.core.userdetails.User;
import org.springframework.stereotype.Component;

@Component
@RequiredArgsConstructor
public class ProjectEntityPermissionHandler implements EntityPermissionHandler {

    @Override
    public boolean hasPermission(User user, Object object, String action) {
        if (object instanceof Project project) {
            if (user.getAuthorities().contains(new SimpleGrantedAuthority("PERMISSION_CREATE"))) return true;
            if (project.getOwner().equals(user.getUsername())) return true;
            if (project.getUsers().stream().anyMatch(usr -> usr.getLogin().equals(user.getUsername()))) return true;
            return user.getAuthorities().contains(new SimpleGrantedAuthority("PROJECT_" + action.toUpperCase()));
        }
        return false;
    }

    @Override
    public String getEntityName() {
        return "PROJECT";
    }
}
